...
class PlatformService(ABC):
    @abstractmethod
    def getTopTrending(self, keyword: str, limit: int = 10) -> List[ExternalItem]:
        pass
...
class YouTubeServiceV2(PlatformService):
    """ YouTube service with async capabilities and process pool for heavy tasks"""
    def __init__(self):
    ...
    async def getTopTrending(self, keyword: str, limit: int = 10) -> List[ExternalItem]:
        ...
        # Offload the request execution to the process pool
        loop = asyncio.get_event_loop()

        response = await loop.run_in_executor(
            thread_pool_manager.get_pool(), 
            self.execute_youtube_request,
            request
        )
    ...
    def execute_upload_video(self, insert_request):
    ...
    async def upload_video_immediate(self, request: UploadVideoInfo):
        try:
            ...
            # Offload heavy operation to process pool
            loop = asyncio.get_event_loop()
            response = await loop.run_in_executor(
                thread_pool_manager.get_pool(), 
                self.execute_upload_video, 
                insert_request
            )
            if response:
    ...
    def fetch_token_wrapper(self, flow, code):
        """Wrapper to fetch token in a thread-safe manner."""
        return flow.fetch_token(code=code)
    async def get_credentials_from_code(self, code: str, redirect_uri: str):
        flow = Flow.from_client_secrets_file(
            self.client_secret_file,
            scopes=['https://www.googleapis.com/auth/youtube.upload'],
            redirect_uri=redirect_uri
    ...
    def execute_youtube_request(self, request):
        """Execute a YouTube API request."""
        return request.execute()
    async def getStatisticsInfoBatch(self, videoIds: List[str]) -> Dict[str, dict]:
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            response = await loop.run_in_executor(thread_pool_manager.get_pool(),
                                                  self.execute_youtube_request,
                                                  request
                                                  )
...
class YouTubeService(PlatformService):
    """ YouTube service without async capabilities, does not use process pool for heavy tasks"""
    def __init__(self):
        self.api_key = get_env_variable('YOUTUBE_API_KEY')
        self.client_secret_file = get_env_variable('YOUTUBE_CLIENT_SECRET_JSON_FILE_PATH')
        self.youtube = build('youtube', 'v3', developerKey=self.api_key)
